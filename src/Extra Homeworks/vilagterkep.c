...
#define SZELESSEG 1080
#define MAGASSAG 540

#define PONTSUGAR 2

#define BME_H 19.05903
#define BME_SZ 47.47341

#define RACSBEOSZTAS_FOKBAN 30.0
#define RACSBEOSZTAS_PIXELBEN ((Sint16)(MAGASSAG * RACSBEOSZTAS_FOKBAN / 360.0))

#define ZOLD 0x7ef57eff
#define KEK 0x2424ddff
#define FEKETE 0x000000ff
#define ATLATSZO_FEKETE ((FEKETE & (~0xff)) | 0x22)
...
void hatter_rajzol(SDL_Renderer *renderer, Uint32 szin)
{
    boxColor(renderer, 0, 0, SZELESSEG, MAGASSAG, szin);
}

void racs_rajzol(SDL_Renderer *renderer, Uint32 szin)
{
    for (Sint16 y = 0; y < MAGASSAG; y += RACSBEOSZTAS_PIXELBEN)
        lineColor(renderer, 0, y, SZELESSEG, y, szin);

    for (Sint16 x = 0; x < SZELESSEG; x += RACSBEOSZTAS_PIXELBEN)
        lineColor(renderer, x, 0, x, MAGASSAG, szin);
}

Sint16 x_lekepez(float h)
{
    return SZELESSEG / 2 + (Sint16)(h * SZELESSEG / 360.0);
}

Sint16 y_lekepez(float sz)
{
    return MAGASSAG / 2 - (Sint16)(sz * MAGASSAG / 180.0);
}

void koordinata_rajzol(SDL_Renderer *renderer, float h, float sz)
{
    Sint16 x = x_lekepez(h);
    Sint16 y = y_lekepez(sz);
    /* sajnos a körökkel nem működik jól a Color-ös verzió, úgyhogy konstans piros */
    filledCircleRGBA(renderer, x, y, PONTSUGAR, 255, 0, 0, 255);
    aacircleRGBA(renderer, x, y, PONTSUGAR, 255, 0, 0, 255);
}

bool koordinatak_lekepez(float const *koordinatak, int db, Sint16 **p_xkoord, Sint16 **p_ykoord)
{
    Sint16 *xkoord = (Sint16*)malloc(db * sizeof(Sint16));
    if (xkoord == NULL)
        return false;
    Sint16 *ykoord = (Sint16*)malloc(db * sizeof(Sint16));
    if (ykoord == NULL)
    {
        free(xkoord);
        return false;
    ...
    for (int i = 0; i < db; i++)
    {
        xkoord[i] = x_lekepez(koordinatak[2 * i + 0]);
        ykoord[i] = y_lekepez(koordinatak[2 * i + 1]);
    }
...
bool alakzatok_rajzol(SDL_Renderer *renderer, float const *koordinatak, Uint32 szin)
{
    int db = 0;
    for (int i = 0; !(koordinatak[i + 0] == -1 && koordinatak[i + 1] == -1); i += 2)
    {
        if (koordinatak[i + 0] == 0 && koordinatak[i + 1] == 0 && db != 0)
        {
            Sint16 *xkoord, *ykoord;
            if (!koordinatak_lekepez(koordinatak + i - 2 * db, db, &xkoord, &ykoord))
                return false;

            filledPolygonColor(renderer, xkoord, ykoord, db, szin);
            aapolygonColor(renderer, xkoord, ykoord, db, FEKETE);
...
int main(int argc, char *argv[])
{
    ...
    sdl_init("Vil\u00e1gt\u00e9rk\u00e9p", SZELESSEG, MAGASSAG, &window, &renderer);

    hatter_rajzol(renderer, KEK);
    if (!alakzatok_rajzol(renderer, vilag, ZOLD) || !alakzatok_rajzol(renderer, tavak, KEK))
        SDL_Quit();
    racs_rajzol(renderer, ATLATSZO_FEKETE);
    koordinata_rajzol(renderer, BME_H, BME_SZ);
    ...
    while (futas)
    {
        ...
        case SDL_MOUSEBUTTONUP:
            if (event.button.button != SDL_BUTTON_LEFT)
                break;
            /* ha a bal klikk felengedése volt, akkor switch fall-through, mert így optimálisabb */
        case SDL_QUIT:
            futas = false;
            break;
        }
